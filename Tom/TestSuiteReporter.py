    ...
    To use:
        ...
        - append a test case to the reporter object
            > reporter.addTestCase(
                "testcasereferencename",  (make sure this string is unique!, this becomes a key value for an internal dictionary)
                "Test Case ID Code",      (good to be unique for reporting purposes)
                "Test Case Decription"
            )

        - report a test step (refer to the declared name of the test, i.e. ("testcasereferencename" here equals "testcasereferencename" above))
...
class TestCase:
    ...
    def __post_init__(self):
        '''Sets up other object values after dataclass initialization'''
        self.steps = []
        self.testStatus = 1
    ...
    def _screenshot(
        ...
        if imagePath == '':
            #TOM ADDED RAND PART
            pathSuffix = description.replace("/", "_").replace("\\", "_") + str(random.getrandbits(10)) + ".png"
        else:
